...
RBCParamSpec: Dict[str, GAParameterSpec] = {
    ...
    'gate_12': IntGAParameterSpec(0, 1),
    'gate_13': IntGAParameterSpec(0, 1),
    'gate_14': IntGAParameterSpec(0, 1),
    'gate_15': IntGAParameterSpec(0, 1),
    'gate_16': IntGAParameterSpec(0, 1),
    'gate_17': IntGAParameterSpec(0, 1),
    'gate_18': IntGAParameterSpec(0, 1),
    'gate_19': IntGAParameterSpec(0, 1)
}
...
def circuit_search(settings):
    ...
    """
    input_values = 4
    output_values = [4, 5, 6, 7]
    gate_set = WIRING[:GATES]
...
def add_search_results(settings, results, dataframe):
    """
    Convert search population metrics and final population values to dataframe rows with circuit_description description
    and parameter settings as well as the truth table output and whether a circuit_description is in the final
    population.
    ...
    """
    input_values = 4
    output_values = [GATES, GATES + 1, GATES + 2, GATES + 3]
    wiring = WIRING[:GATES]
...
if __name__ == "__main__":
    ...
    for i in range(0, 2 ** GATES):
        ...
        for bit in str(b):
            if bit == "1":
                circuit_description.append("OR")
            else:
                circuit_description.append("AND")
        circuit_settings.append(circuit_description)
    ...
    save_results(df, str(GATES) + "_gate_test_data.csv")
